...
export default function DCProductDetails() {
  ...
  useEffect(() => {
    const open = (event: Event) => { const selected = products[(event as CustomEvent<string>).detail]; if (!selected) return; setProduct(selected); setSlide(0); setIngredientIndex(0); document.body.classList.add('dc-detail-open'); };
  ...
  const selectedIngredient = product.ingredients[ingredientIndex] ?? product.ingredients[0];
    ...
    <div className="dc-detail-shell">
      ...
      <section className="dc-detail-hero">
        ...
        <div className="dc-detail-intro">
          ...
          <div className="dc-benefit-pills"><span>WHAT IT IS</span><span>WHY IT'S HERE</span><span>HOW TO USE IT</span></div>
          ...
          <p className="dc-disclaimer">*Product claims and pack statements should be shown only after final formula/label verification.</p>
        </div>
      </section>

      <section className="dc-education-intro">
        <p className="dc-detail-kicker">KNOW YOUR DAILY FLEX</p>
        <h2>What’s inside — and <i>why.</i></h2>
        <p>Instead of a long ingredient wall, explore the formula by purpose. Tap an ingredient to learn what it is and why it appears in this DC product.</p>
      </section>

      <section className="dc-formula-map">
        {product.groups.map((group, index) => <article key={group.title} className={`dc-formula-card dc-formula-${index % 4}`}>
          <div className="dc-formula-orbit"><span>{index + 1}</span></div>
          <small>{group.title}</small>
          <h3>{group.names.join(' • ')}</h3>
          <p>{group.explanation}</p>
        </article>)}
      </section>

      <section className="dc-ingredient-spotlight">
        <div className="dc-spotlight-copy">
          <p className="dc-detail-kicker">INGREDIENT SPOTLIGHT</p>
          <h2>{selectedIngredient.name}</h2>
          <p className="dc-spotlight-role">{selectedIngredient.role}</p>
          <div className="dc-spotlight-block"><small>WHAT IT DOES</small><p>{selectedIngredient.benefit}</p></div>
          {selectedIngredient.amount && <div className="dc-spotlight-block"><small>FORMULA AMOUNT</small><p>{selectedIngredient.amount}</p></div>}
        </div>
        <div className="dc-ingredient-picker" aria-label="Choose an ingredient to learn about">
          {product.ingredients.map((ingredient, index) => <button type="button" key={ingredient.name} className={index === ingredientIndex ? 'active' : ''} onClick={() => setIngredientIndex(index)}><span>{String(index + 1).padStart(2,'0')}</span><strong>{ingredient.name}</strong><small>{ingredient.role}</small></button>)}
        </div>
      </section>

      <section className="dc-compact-info">
        <details><summary>FULL INGREDIENT LIST <span>+</span></summary><div className="dc-full-list">{product.ingredients.map(ingredient => <span key={ingredient.name}>{ingredient.name}{ingredient.amount ? ` — ${ingredient.amount}` : ''}</span>)}</div></details>
